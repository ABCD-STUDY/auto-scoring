        ...
        request({
            ...
            return function (error, response, body) { // in case we have an error, try to send again one after another
                if (error || response.statusCode !== 200) {
                    ...
                    var errorParts = [];
                    if (typeof response !== 'undefined' && typeof response['body'] !== 'undefined' && typeof response['body']['error'] !== 'undefined') {
                        errorParts = response['body']['error'].match(/NDAR_INV[^"]+/g);
                        errorParts = [...new Set(errorParts)]; // unique
                        console.log("found participant IDs that caused errors and should be removed: " + JSON.stringify(errorParts));
                    }
